...
class Echo(protocol.Protocol):
	def dataReceived(self, data):
		...
		if os.stat('attendance.csv').st_size > 40000:
			...
			output = commands.getoutput("mysqlimport --ignore-lines=0 --fields-terminated-by=, --columns='timestamp,rssi,id,sensor_id' --local -u root -p'the power to do what is right' cattalax ./attendance.csv >> datalog.txt")
			if 'cattalax.attendance: Records' in output:
				os.remove('attendance.csv')
				os.system("echo "+output+" >> datalog.txt")
